# 📱 Коллекция макросов MacroDroid

[![Readme EN](https://img.shields.io/badge/README-EN-blue.svg)](https://github.com/BespredeL/MacroDroid/blob/main/README.md)
[![Readme RU](https://img.shields.io/badge/README-RU-blue.svg)](https://github.com/BespredeL/MacroDroid/blob/main/README_RU.md)
[![GitHub license](https://img.shields.io/badge/license-MIT-458a7b.svg)](https://github.com/BespredeL/MacroDroid/blob/main/LICENSE)

Подборка мощных и гибко настраиваемых макросов для [MacroDroid](https://www.macrodroid.com), предназначенных для автоматизации Android-устройств.

Каждый макрос размещён в отдельной папке и включает:

- Файл макроса для прямого импорта
- Документ `README_RU.md` с подробным описанием, списком возможностей и changelog

---

## 📦 Список макросов

### 🤖 [Простой Telegram-бот](./SimpleTelegramBot/)

Удалённое управление Android-устройством через Telegram.

- 🔐 Авторизация по Telegram ID
- 📍 Получение геолокации, информации об устройстве и подача сигнала
- 🔁 Пересылка уведомлений и SMS
- 📸 Фото при попытке несанкционированного доступа
- 📞 Управление звонками (отклонение / вызов на указанный номер)
- 🧠 Поддержка AI через DeepSeek (по желанию)
- 🌐 Проверка ссылок через VirusTotal (по желанию)
- 🗂️ Сохранение медиа, документов и заметок
- 🌍 Многоязычный интерфейс

📄 **[Подробное описание →](./SimpleTelegramBot/README_RU.md)**

📲 **[Ссылка на макрос](https://www.macrodroidlink.com/macrostore?id=25355)**

---

### 🔦 [Включение фонарика по встряхиванию](./ShakeFlashlight/)

Автоматическое включение фонарика при встряхивании устройства в условиях низкой освещённости.

- 🌑 Срабатывает при освещении < 50 люкс
- 🔒 Требуется заблокированный, но включённый экран
- 📏 Датчик приближения должен быть свободен
- ⚙️ Возможность настройки чувствительности встряхивания

📄 **[Подробнее →](./ShakeFlashlight/README_RU.md)**

📲 **[Ссылка на макрос](https://www.macrodroidlink.com/macrostore?id=134)**

---

### 🤖 [Родительский контроль](./ParentalControl/)

Макрос предназначен для родительского контроля. Он собирает данные с устройства ребёнка, передавая их на устройство родителя:

- 📞 Входящие и исходящие звонки
- ✉️ SMS сообщения
- 🔔 Уведомления приложений
- 📍 GPS координаты
- 📶 Историю Wi-Fi подключений
- 🌍 Легкий перевод через переменную `langs`

📄 **[Подробное описание →](./ParentalControl/README_RU.md)**

📲 **[Ссылка на макрос](https://www.macrodroidlink.com/macrostore?id=27550)**

---

### 💡 [Включение ПК](./WakePC/)

Этот макрос позволяет включить компьютер или другое устройство в локальной сети с помощью технологии Wake-on-LAN (WoL).
Достаточно подключиться к той же Wi-Fi сети, указать MAC-адрес и broadcast-адрес устройства - и MacroDroid отправит специальный *Magic Packet*,
который запустит устройство.

- 🖥️ Автоматическая/ручная отправка WOL-пакета
- 🌐 Поддержка broadcast и авто-поиска адреса
- 📡 Возможность пробуждать устройство из других макросов или виджетов
- 🔧 Поддержка пользовательских настроек MAC-адреса и сетевого сегмента

📄 **[Подробное описание →](./WakePC/README_RU.md)**

📲 **[Ссылка на макрос](https://www.macrodroidlink.com/macrostore?id=27708)**

---

### ⏩ [AutoSkip YouTube Ads](./AutoSkipYouTubeAds/)

Автоматически пропускает рекламу в YouTube и YouTube Shorts.

- ⏩ Автоматический пропуск рекламы
- 🔇 Отключение звука во время рекламных вставок
- 📱 Поддержка обычных видео YouTube
- 🎬 Поддержка YouTube Shorts
- 🌍 Поддержка нескольких языков
- ⚡ Временная оптимизация сканирования экрана для точного обнаружения
- ⚙️ Использует службу специальных возможностей MacroDroid UI Interaction

📄 **[Подробное описание →](./AutoSkipYouTubeAds/README_RU.md)**

📲 **[Ссылка на макрос](https://www.macrodroidlink.com/macrostore?id=27954)**

---

### 📸 [Фото злоумышленника](./IntruderCam/)

Этот макрос автоматически фиксирует любые попытки несанкционированного доступа и собирает ключевые доказательства.

- 🤳 Делает снимки с фронтальной и основной камер при неудачных попытках разблокировки (по умолчанию - 2 попытки за 10 минут).
- 🗺️ Сохраняет GPS-координаты в виде HTML-страницы с интерактивной картой.
- 🎥 Записывает 30 секунд видео с фронтальной камеры.
- 🔔 После успешной разблокировки показывает уведомление о нарушении.

📄 **[Подробное описание →](./IntruderCam/README_RU.md)**

📲 **[Ссылка на макрос](https://www.macrodroidlink.com/macrostore?id=27640)**

---

### 🛡️ [Анти-мошенник](./AntiScamProtector/)

Защита от телефонных мошенников, рассылающих коды во время звонка.

- 📴 Скрывает SMS/уведомление с кодом
- 📩 Отправляет тревожное сообщение на доверенный номер
- 🚨 Показывает уведомление о возможном мошенничестве
- ⚠️ Открывает окно с предупреждением и двумя кнопками: сброс вызова или закрытие
- ⏱️ Через 30 секунд - автоматический сброс вызова
- 🔄 Повторный показ окна для усиления эффекта
- 🌍 Легкий перевод через переменную `langs`

📄 **[Подробнее →](./AntiScamProtector/README_RU.md)**

📲 **[Ссылка на макрос](https://www.macrodroidlink.com/macrostore?id=25801)**

---

### 📁 [Веб-файловый менеджер](./RemoteFilemanager/)

Позволяет управлять файлами устройства через браузер - полностью локально, без облака.

- 🌐 Доступ к файлам через браузер
- 📂 Навигация по папкам
- 📄 Просмотр содержимого директорий
- 📊 Информация о файлах (размер, дата)
- 🗑️ Удаление файлов и папок
- 📁 Создание папок
- 📋 Копирование файлов
- ✂️ Перемещение файлов
- 🌍 Поддержка нескольких языков
- 🔐 Работа в локальной сети (Wi-Fi / hotspot)

📄 **[Подробнее →](./RemoteFilemanager/README_RU.md)**

📲 **[Ссылка на макрос](https://www.macrodroidlink.com/macrostore?id=29850)**

---

### 🤝 [ShakeContacts](./ShakeContacts/)

Быстрая передача контактной информации и профилей социальных сетей с помощью QR-кодов.

- 🤳 Запуск встряхиванием устройства
- ☎️ Передача телефонных контактов в формате vCard
- 🌐 Передача сайтов и профилей социальных сетей
- 💬 Поддержка WhatsApp и Telegram
- 🔗 Поддержка множества социальных сетей и сервисов
- 📱 Отображение QR-кодов прямо на устройстве
- 💾 Локальное сохранение QR-кодов
- 🌍 Многоязычный интерфейс

📄 **[Подробное описание →](./ShakeContacts/README_RU.md)**

---

### 🎚️ [Fast Media Vol.](./FastMediaVol/)

Быстрое отключение и восстановление громкости медиа с помощью аппаратных кнопок громкости.

- 🔇 Отключение звука долгим нажатием Volume Down
- 🔊 Восстановление предыдущей громкости долгим нажатием Volume Up
- 💾 Автоматическое сохранение предыдущего уровня громкости
- 📢 Отображение текущего состояния звука
- ⚡ Быстрое управление аппаратными кнопками
- 🌍 Многоязычные сообщения

📄 **[Подробное описание →](./FastMediaVol/README_RU.md)**

---

> ℹ️ Новые макросы будут добавляться - следите за обновлениями.

---

## 🛠 Инструкция по использованию

1. Откройте MacroDroid на вашем Android-устройстве
2. Импортируйте макрос из файла `.macro` в соответствующей папке
3. Прочитайте `README.md` внутри папки для подробной настройки
4. При необходимости адаптируйте логику, триггеры и переменные под свои нужды

---

## 💬 Обратная связь и вклад

Есть идея, нашли баг или хотите предложить улучшение?  
Создайте issue или отправьте pull request - ваш вклад приветствуется.

---

## 🧩 О репозитории

Данный репозиторий - это коллекция продвинутых шаблонов автоматизации MacroDroid.

Он идеально подойдёт для:

- 🔐 Безопасности и защиты от краж
- 📦 Удалённого управления устройством
- 🧠 Интеграции AI-ассистентов
- 📡 Перенаправления уведомлений
- 🔧 Изучения и создания собственных сценариев автоматизации

---

## ❤️ Поддержка

Если макросы оказались полезными — поставь ⭐ звезду и поделись репозиторием!

---

📣 **Есть вопросы или идеи?**  
Пишите на: [hello@bespredel.name](mailto:hello@bespredel.name)
